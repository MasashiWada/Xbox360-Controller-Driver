		...
		if( TargetDeviceFound == TRUE )
		{
			switch( IoMode )
			{
				...
				case IO_MODE_GET_STATE:
					...
					Report.Id	= GetControllerId( Detail->DevicePath );
				...
				case IO_MODE_SET_STATE:
					//	デバイスの状態を設定する
					DWORD	ReqLen;
					WriteReport->Id	= GetControllerId( Detail->DevicePath );
					WriteFile(
						 Device
						,WriteReport
...
//	GetControllerId
//----------------------------------------------------------------------------------------------
CHAR CDevice::GetControllerId( WCHAR * DevicePath )
{
	//	変数宣言
	CHAR	ControllerId;
	...
	//	コントローラの ID を取得する
	ControllerIdPos	= wcsstr( DevicePath, L"&col0" );
