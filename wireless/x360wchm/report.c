...
//	HID Minidriver for Microsoft Xbox 360 Wireless Receiver
...
VOID SetAxis( IN SHORT, IN UCHAR, IN BOOLEAN, IN UCHAR, OUT SHORT *, IN OUT BOOLEAN *, OUT LARGE_INTEGER *, IN LARGE_INTEGER );
VOID SetSlider( IN SHORT, IN UCHAR, IN BOOLEAN, IN UCHAR, OUT SHORT *, IN OUT BOOLEAN *, OUT LARGE_INTEGER *, IN LARGE_INTEGER );
VOID SetButton( IN BOOLEAN, IN UCHAR, IN BOOLEAN, IN UCHAR, OUT ULONG *, IN OUT BOOLEAN *, OUT LARGE_INTEGER *, IN LARGE_INTEGER );
				...
				for( WayIndex = INDEX_WAY_UP; WayIndex <= INDEX_WAY_RIGHT; WayIndex ++ )
				{
					...
					&&	Setting->StickWayUsage[Index][WayIndex] <= USAGE_Y_ROTATION_PLUS )
					{
						SetAxis(
							...
							,&LastState->StickWayRapidFireTime[Index][WayIndex]
							,ReportTime );
					}
					//	軸 (マイナス)
			...
			for( WayIndex = INDEX_WAY_UP; WayIndex <= INDEX_WAY_RIGHT; WayIndex ++ )
			{
				...
				&&	Setting->DirectionalPadWayUsage[WayIndex] <= USAGE_Y_ROTATION_PLUS )
				{
					SetAxis(
						...
						,&LastState->DirectionalPadWayRapidFireTime[WayIndex]
						,ReportTime );
				}
				//	軸 (マイナス)
	...
	for( Index = INDEX_LEFT_TRIGGER; Index <= INDEX_RIGHT_TRIGGER; Index ++ )
	{
		...
		&&	Setting->TriggerUsage[Index] <= USAGE_Y_ROTATION_PLUS )
		{
			SetAxis(
				...
				,&LastState->TriggerRapidFireTime[Index]
				,ReportTime );
		}
		//	軸 (マイナス)
	...
	for( Index = INDEX_A_BUTTON; Index <= INDEX_XBOX_GUIDE_BUTTON; Index ++ )
	{
		...
		&&	Setting->ButtonUsage[Index] <= USAGE_Y_ROTATION_PLUS )
		{
			SetAxis(
				...
				,&LastState->ButtonRapidFireTime[Index]
				,ReportTime );
		}
		//	軸 (マイナス)
...
VOID SetAxis(
	...
	,OUT LARGE_INTEGER *	RapidFireTime
	,IN LARGE_INTEGER		ReportTime )
{
	//	変数宣言
	...
	if( RapidFire == FALSE )
	{
		...
		if( Value == TRUE )
		{
			...
			{
				Axis[Usage]	= ADD_SHORT( Axis[Usage], Value );
				KeQuerySystemTime( RapidFireTime );
				*LastState	= TRUE;
			} else {
				Interval.QuadPart	= ReportTime.QuadPart - RapidFireTime->QuadPart;
				UpTime.QuadPart		= 2400 * 1000 * 10 / max( 1, RapidFireRate );
				if( Interval.QuadPart < 24 * 1000 * 10 )
				{
					Axis[Usage]	= ADD_SHORT( Axis[Usage], Value );
				} else {
					if( UpTime.QuadPart < Interval.QuadPart )
					{
						RapidFireTime->QuadPart	+= (UpTime.QuadPart);
					}
				}
			}
		} else {
		...
		case REPORT_TYPE_NORMAL:
			if( Setting->Actuator == TRUE )
			{
				...
				SetActuator(
					...
					,&OutputReport->LeftActuator );
				//	右アクチュエータ
				SetActuator(
					 Report->RightActuator
					,Setting->ActuatorLevel[INDEX_RIGHT_ACTUATOR]
					,Setting->ActuatorMinThreshold
					,Setting->ActuatorMaxThreshold
					,&OutputReport->RightActuator );
			} else {
				OutputReport->LeftActuator	= 0;
				OutputReport->RightActuator	= 0;
			}
			break;

		//	調整なし
		case REPORT_TYPE_DIRECT:
			OutputReport->LeftActuator	= Report->LeftActuator;
			OutputReport->RightActuator	= Report->RightActuator;
			break;
	}
}
