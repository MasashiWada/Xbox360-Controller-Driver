...
#define	REGISTRY_PATH				L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\Xbox360WirelessController\\Parameters"
...
typedef struct _LAST_STATE
{
	...
	LARGE_INTEGER	StickWayRapidFireTime[2][4];
	BOOLEAN			DirectionalPadWayLastState[4];
	LARGE_INTEGER	DirectionalPadWayRapidFireTime[4];
	BOOLEAN			TriggerLastState[2];
	LARGE_INTEGER	TriggerRapidFireTime[2];
	BOOLEAN			ButtonLastState[11];
	LARGE_INTEGER	ButtonRapidFireTime[11];
...
typedef struct _DEVICE_EXTENSION
{
	...
	//	ワーカ スレッド
	BOOLEAN							ThreadStateIo;
	PVOID							ThreadObjectIn[MAX_CONTROLLER_COUNT];
	KEVENT							ThreadExitingIn[MAX_CONTROLLER_COUNT];
	PVOID							ThreadObjectOut[MAX_CONTROLLER_COUNT];
	KEVENT							ThreadExitingOut[MAX_CONTROLLER_COUNT];
	THREAD_PARAM					ThreadParam[MAX_CONTROLLER_COUNT];

	//	IRP
	BOOLEAN							ThreadStateIdc;
	PVOID							ThreadObjectIdc;
	KEVENT							ThreadExitingIdc;
	KSPIN_LOCK						LockIrp;
	PIRP *							Irps;
	LONG							IrpCount;
	KEVENT							IoReported;
	...
	USBD_PIPE_HANDLE				InputPipeHandle[MAX_CONTROLLER_COUNT];
	USBD_PIPE_HANDLE				OutputPipeHandle[MAX_CONTROLLER_COUNT];

	//	レポート
	KSPIN_LOCK						LockReport;
	INPUT_REPORT					ControllerReport[MAX_CONTROLLER_COUNT];
	LARGE_INTEGER					ControllerReportTime[MAX_CONTROLLER_COUNT];
	UCHAR							ReadReportId;
	INPUT_REPORT					CurrentControllerReport[MAX_CONTROLLER_COUNT];
	LARGE_INTEGER					CurrentControllerReportTime[MAX_CONTROLLER_COUNT];
	LARGE_INTEGER					CurrentReadReportTime;
	INPUT_REPORT					LastControllerReport[MAX_CONTROLLER_COUNT];
	LARGE_INTEGER					LastControllerReportTime[MAX_CONTROLLER_COUNT];
	LARGE_INTEGER					LastReadReportTime;
	OUTPUT_REPORT					ConnectionReport[MAX_CONTROLLER_COUNT];
	OUTPUT_REPORT					ActuatorsReport[MAX_CONTROLLER_COUNT];
	OUTPUT_REPORT					RingLightReport[MAX_CONTROLLER_COUNT];
	BOOLEAN							Connected[MAX_CONTROLLER_COUNT];
	BOOLEAN							ConnectionReported[MAX_CONTROLLER_COUNT];
	BOOLEAN							ActuatorsReported[MAX_CONTROLLER_COUNT];
	BOOLEAN							RingLightReported[MAX_CONTROLLER_COUNT];

	//	設定
	BOOLEAN							ResetDevice[MAX_CONTROLLER_COUNT];
	SETTING							Setting;

	//	連射
	LAST_STATE						LastState[MAX_CONTROLLER_COUNT];
...
NTSTATUS InputReport( IN PDEVICE_OBJECT, IN USBD_PIPE_HANDLE, OUT INPUT_REPORT *, IN LONG );
NTSTATUS OutputReport( IN PDEVICE_OBJECT, IN USBD_PIPE_HANDLE, OUT OUTPUT_REPORT *, IN LONG );

//	internal_device_control.c
__drv_dispatchType(IRP_MJ_INTERNAL_DEVICE_CONTROL) DRIVER_DISPATCH InternalDeviceControl;
VOID ReadReport( IN PIRP, IN PDEVICE_EXTENSION );

//	setting.c
VOID LoadSetting( IN SETTING * );

//	report.c
VOID GenerateReadReport( IN INPUT_REPORT *, IN SETTING *, OUT HID_READ_REPORT *, IN OUT LAST_STATE *, IN LARGE_INTEGER );
